            ...
            <div class="col-md-8 col-lg-6">
                ...
                <div>
                    <?php foreach ($tasks as $task): ?>
                        <div class="task-card shadow-sm mb-3" data-task-id="<?php echo $task['id']; ?>">
                            <div class="d-flex justify-content-between align-items-center">
                                <div class="d-flex align-items-center">
                                    <input type="checkbox" class="task-checkbox" 
                                        data-task-id="<?php echo $task['id']; ?>" 
                                        onchange="toggleTaskStatus(this)" 
                                        <?php echo (isset($task['status']) && $task['status'] === 'completado') ? 'checked' : ''; ?>>
                                    <h5 class="task-title ms-3 <?php echo (isset($task['status']) && $task['status'] === 'completado') ? 'completed' : ''; ?>">
                                        <?php echo htmlspecialchars($task['task']); ?>
                                    </h5>
                                </div>
                                <div>                                  
                                    <a href="index.php?delete_task=<?php echo $task['id']; ?>" class="btn btn-sm btn-outline-danger">Deletar</a>
                                    <button class="btn btn-sm btn-edit-custom" data-bs-toggle="modal" data-bs-target="#editModal<?php echo $task['id']; ?>">Editar</button>
                                </div>
                            </div>
                            <p class="due-date" style="margin-left: 30px;">Prazo: <?php echo htmlspecialchars($task['due_date']); ?></p>
                        </div>
